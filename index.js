...
app.get('/', homeLimiter, (req, res) => {
  res.sendFile(path.join(__dirname, 'Assets', 'xylo.html'))
})
...
function startBot() {
  ...
  console.log(chalk.blue('Starting XYLO Bot with:'))
  ...
  const args = [path.join(path.dirname(currentFilePath), 'Xylo.js'), ...process.argv.slice(2)]
