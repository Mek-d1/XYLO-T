...
if (!conn.authState.creds.registered) {
  ...
  if (phoneNumberFromEnv) {
    ...
    if (!phoneNumber || phoneNumber.length < 8) {
      console.log(
        chalk.bgBlack(chalk.redBright("Invalid phone number format. Please include country code (Example: 234xxx)"))
      )
      if (process.send) {
        process.send({ 
          type: 'pairing-code', 
  ...
  setTimeout(async () => {
    try {
      let code = await conn.requestPairingCode(phoneNumber, "XYLOARCH")
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    try {
      ...
      const msg = `Hai🤩 ${name}, Congrats you have successfully deployed XYLO\nJoin my support Channel for any Query\n https://whatsapp.com/channel/0029VarIiQL5a24AU5ZCVV0G`
